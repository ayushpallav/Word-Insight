    ...
    if(rqst.readyState == 4)
    {
        if(rqst.status == 200)
        {
          ...
          chrome.extension.onConnect.addListener(function(port) {
              ...
              port.postMessage("<b style='color:#191970;'>" + word + "</b> :<br> " + responseBody.results[0].lexicalEntries[0].entries[0].senses[0].definitions[0]);
            });
          });
                  ...
                  function iChangeIN()
                  {
                    if(rst.readyState == 4)
                    {
                      if(rst.status == 200)
                      {
                        ...
                        chrome.extension.onConnect.addListener(function(port) {
                            port.onMessage.addListener(function(msg) {
                            port.postMessage("<b style='color:#191970;'>" + key + "</b> : <br>" + responseBody.results[0].lexicalEntries[0].entries[0].senses[0].definitions[0]);
                          });
                        });
